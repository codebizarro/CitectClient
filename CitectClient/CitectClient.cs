...
namespace System.Net.CitectClient
{
    public class CitectClient : ICitectClient
    {
        ...
        {
            var query = TrendHelpers.Query(tag, dateRight, GetSamplePeriod(tag), length);
        ...
        {
            var query = TrendHelpers.Query(tag, dateRight, GetSamplePeriod(tag), length, interpolate, legacy);
